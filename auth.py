...
def gitlab_login():
    ...
    if resp.ok:
        ...
        if not user:
            ...
            db.session.add(user)
            db.session.commit()
            flash('GitLab account linked successfully! Your account is pending administrator approval before you can log in.')
            return redirect(url_for('auth.login'))
        if not user.approved:
            flash('Your account is pending administrator approval. Please try again later.')
            return redirect(url_for('auth.login'))
        login_user(user)
        return redirect(url_for('index'))
    return redirect(url_for('auth.login'))
...
def local_login():
    ...
    if form.validate_on_submit():
        user = User.query.filter_by(username=form.username.data, provider='local').first()
...
def signup():
    ...
    if form.validate_on_submit():
        ...
        try:
            db.session.add(user)
            db.session.commit()
            flash('Account created successfully! Your account is pending administrator approval before you can log in.')
            return redirect(url_for('auth.login'))
        except Exception:
            db.session.rollback()
