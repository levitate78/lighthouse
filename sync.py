            ...
            for group_id in group_ids:
                try:
                    ...
                    for proj in projects:
                        ...
                        project.namespace = proj_data.get('namespace', {}).get('full_path', '')
                        ...
                        pipelines = gl.projects.get(proj_data['id']).pipelines.list(per_page=10)
                        ...
                        for pipe in pipelines:
                            ...
                            if recent_pipeline_id and pipe.id == recent_pipeline_id:
                                jobs = gl.projects.get(proj_data['id']).pipelines.get(pipe_data['id']).jobs.list()
                                PipelineJob.query.filter_by(pipeline_id=pipe_data['id']).delete()
                                for job in jobs:
                                    job_data = job.asdict()
                                    job_obj = PipelineJob(
                                        id=job_data['id'],
                                        ...
                                        runner_name=job_data.get('runner', {}).get('description', '') if job_data.get('runner') else '',
                                    )
